"""
Spicy training: multiplies spicy cores into ~300 texting variants each,
then trains (mood + template reply, single rebuild + save at end).

Appends ~11,000 lived desire episodes tagged [spicy] so RAG retrieval finds
relevant past heat when he's horny (spicy mood: spicy/romantic labels).

All cores are desire content, so arousal is preset (heat=3) per turn —
labels stay truthful even when a typo'd variant drops the trigger word.

Idempotent: drops the previous [spicy] batch before regenerating.
Run:  python3 train_spicy.py
"""
import json
import os
import sys
...
from scenarios_spicy import SPICY_CORES
...
def build_variants() -> list:
    ...
    seen, out = set(), []
    for core, grp in SPICY_CORES:
        made, tries = 0, 0
        while made < PER_CORE and tries < PER_CORE * 8:
            tries += 1
            v = ft.variant(core, "warm")
            key = v.lower()
...
def main():
    ...
    # idempotent: drop previous [spicy] batch before regenerating
    store.docs = [d for d in store.docs if not str(d.get("bf", "")).startswith("[spicy]")]
    print(f"Existing RAG docs: {len(store.docs)}")

    variants = build_variants()
    print(f"Generated {len(variants)} spicy variants")
    variants = variants[:TARGET]
    print(f"Training {len(variants)} spicy episodes...")

    engine = MoodEngine(initial_score=86.0)
    last_reply = ""
    counts = {}
    for n, (bf, grp) in enumerate(variants, 1):
        engine.state.score = 86.0 + ft.rng.uniform(-3, 3)  # desire band
        engine.state.heat = 3.0  # all cores are desire content
        ...
        store.docs.append({
            ...
            "bf": f"[spicy] {bf}", "gf": reply, "mood": mood["label"],
    ...
    print(f"\n--- SPICY TRAINING COMPLETE in {time.time()-t0:.0f}s ---")
    print(f"RAG docs: {len(store.docs)}")
    print(f"Spicy mix this run: {dict(counts)}")
    print(f"Spicy-tagged docs: {sum(1 for d in store.docs if str(d.get('bf','')).startswith('[spicy]'))}")
